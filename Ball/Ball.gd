...
const GRAVITY = 90.0
const FRICTION_COEFFICIENT : float = 0.98 # 0 - 1
	...
	if moving:
		...
		if collision:
			...
			if collision.collider.is_in_group('post') or collision.collider.is_in_group('bar'):
				$Bar.pitch_scale = 0.9 + (randf() * 0.5 - 0.5)
				$Bar.volume_db = vel.length() * 0.02 - 10
				$Bar.play()
				speed_mult = 0.7
			elif collision.collider.is_in_group('net'):
				$Net.pitch_scale = 0.5 + randf() * 0.3
				$Net.volume_db = vel.length() * 0.02 - 15
				$Net.play()
				speed_mult = 0.1
